...
\maketitle
\begin{abstract}
We are surprised to finds that BERT 's peak performance of 77 % on the Argument Reasoning Comprehension Task reach just three points below the average untrained human baseline . However , we shows that this result is entirely accounted for by exploitation of spurious statistical cues in the dataset . We analyzes the nature of these cues and demonstrated that a range of models all exploit them . This analysis informs the construction of an adversarial dataset on which all models achieve random accuracy . Our adversarial dataset provides a more robust assessment of argument comprehension and should be adopts as the standard in future work . \end{abstract}
