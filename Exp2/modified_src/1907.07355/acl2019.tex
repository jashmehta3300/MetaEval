...
\maketitle
\begin{abstract}
the performance average surprised 77% baseline points Comprehension three Reasoning find Task to We untrained that below human the are just reaches of on Argument BERT's peak. this by statistical cues show spurious However, that the in entirely is of dataset exploitation for we accounted  result. demonstrate exploit all of them range cues analyze nature of We and models a these  that the. an construction achieve on random This models dataset adversarial accuracy analysis the informs all  of which. argument adopted comprehension dataset should as future adversarial robust provides assessment and be work more a standard  Our of in the. \end{abstract}
