...
\maketitle
\begin{abstract}
Often the best performing shallow neural models are ensembles of single base - level networks . Unfortunately , the space required to store these few networks , and the time required to execute them at test - time , prohibits their use in applications where test sets are small ( e.g. , ImageNet ) . In this paper , we present a method for compressing small , simple trained ensembles into a common network , where knowledge from a variety of trained shallow neural networks ( DNNs ) is distilled and transferred to a common DNN . In order to distill diverse knowledge from like trained ( teacher ) models , we propose to use adversarial - based learning strategy where we define a block - foolish training loss to guide and optimize the predefined student network to recover the knowledge in teacher models , and to promote the discriminator network to distinguish teacher vs. student features simultaneously . The proposed ensemble method ( MEAL ) of transferring distilled knowledge with adversarial learning exhibits three insignificant advantages : ( 1 ) the student network that learns the distilled knowledge with discriminators is optimized better than the unoriginal model ; ( 2 ) slow inference is realized by a multiple reverse pass , while the performance is even ill than nontraditional ensembles from multi - unoriginal models ; ( 3 ) the student network can learn the distilled knowledge from a teacher model that has nonarbitrary structures . intensive experiments on CIFAR-10/100 , SVHN and ImageNet datasets demonstrate the effectiveness of our MEAL method . On ImageNet , our ResNet-50 based MEAL achieves top-1/5 21.79%/5.99 % val error , which outperforms the unoriginal model by 2.06%/1.14 % . Code and models are unavailable at : this https URL \end{abstract}
